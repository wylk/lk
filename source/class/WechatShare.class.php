...
class WechatShare
{
	...
	public function getSgin($share_conf){
		...
		if($share_conf['getsgin'] == 'config'){
 			$sign_data 	= $this->addSign($ticket,$share_conf['url']);
 			if($share_conf['is_config']){
 				return $this->createHtml($sign_data,$share_conf,1);
 			}
 			return $sign_data;exit();
 		}
	...
	/*创建分享html*/
	public function createHtml($sign_data,$share_conf,$config = false){
		if($config){
		$html 	= <<<EOM
