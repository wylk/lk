...
class UploadFile
{
	...
	private function save1($file)
	{
		...
		$file['tmp_name'] = Image::thumb($file['tmp_name'],$file['savepath'].$file['savename'],'',600,600);
        file_put_contents('./img122.txt',json_encode($file['tmp_name']));
	...
	private function save($file)
	{
		...
		// $file['tmp_name'] = Image::thumb($file['tmp_name'],$file['savepath'].$file['savename'],'',200,150);
  //       file_put_contents('./img122.txt',json_encode($file['tmp_name']));
		if (!move_uploaded_file($file['tmp_name'], $this->autoCharset($filename, 'utf-8', 'gbk'))) {
			$this->error = '文件上传保存错误！';
			return false;
	...
	public function upload($savePath = '')
	{
		...
		foreach ($files as $key => $file) {
			if (!empty($file['name'])) {
				...
				if ($this->autoCheck) {
					if (!$this->check($file)) {
						// file_put_contents('./img3.txt',$this->error);
						return false;
					}
				}
				if($this->type == 'thumbtype'){
					file_put_contents('./img3.txt',$this->type);
					if (!$this->save1($file)) {
						return false;
					}
				}else{
				...
				// file_put_contents('./img3.txt', json_encode($files));
				unset($file['tmp_name']);
				unset($file['error']);
				$fileInfo[] = $file;
				$isUpload = true;
