...
class PlatformCurrency{
    ...
    public function currency(){
        ...
        if(count($this->matchOrderData) > 0){
            foreach($this->matchOrderData as $key=>$value){
                $this->matchOrderData[$key]['tran_other'] = $this->tranId;
            }
    ...
    public function addRegister($type){
        ...
        $this->tranId = D("Card_transaction")->data($tranData)->add();
        $tranData['id'] = $this->tranId;
...
// 自动匹配相应的订单
    ...
    public function createOrder($data,$type){
        ...
        if($tranList['uid'] == $this->userId)    return ['res'=>1,"此订单为本人发布"];
        if($tranList['status'] != 0)  return ['res'=>1,"此交易单已关闭"];

        $packageInfo = D("Card_package")->where(['id'=>$this->packageId])->find();
        if($packageInfo['num'] < ($tranList['num']-$tranList['frozen']))  return ['res'=>1,"现有金额不足","other"=>$packageInfo['num']];
    ...
    public function selectRegister($data){
        // 查询当前卖单信息
        $list = D("Card_transaction")->where(['uid'=>['not in',[$data['userId']]],'card_id'=>$data['cardId'],"type"=>$data['type'],"status"=>'0'])->order("create_time desc")->select();
        return $list;
    }
    // 交易记录列表
