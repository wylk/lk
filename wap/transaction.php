...
if(isset($_POST['type']) && $_POST['type'] == "transaction"){
	...
	$cardBagInfo = D("Card_package")->field("address,is_publisher,frozen")->where(['uid'=>$userId,'card_id'=>$cardId])->find();
	...
	if(D("Card_transaction")->data($data)->add()){
		$frozenData['frozen'] = array_sum($cardBagInfo['frozen'],$num);
		$res = D("Card_package")->data($frozenData)->where(['uid'=>$userId,'card_id'=>$cardId])->save();
		$res ? true : dexit(['res'=>1,"msg"=>"冻结修改失败"]);
		dexit(['res'=>0,"msg"=>"卡券发布成功"]);
	}
	dexit(['res'=>1,"msg"=>"卡券发布失败",'other'=>$data]);
}
