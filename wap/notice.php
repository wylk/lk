...
$sendAddress = D('Card_package')->field('address')->where(['uid'=>$order['sole_id'],'card_id'=>$order['card_id']])->find();
$getAddress = D('Card_package')->field('address')->where(['uid'=>$order['buy_id'],'card_id'=>$order['card_id']])->find();
// 记录账单信息
import("AccountBook");
$Account_book = new AccountBook();
$bookJson = json_encode(['uid'=>$order['sole_id'],"contract_id"=>$order['card_id'],'sendAddress'=>$sendAddress['address'],"num"=>$order['number'],"getAddress"=>$getAddress['address']]);
